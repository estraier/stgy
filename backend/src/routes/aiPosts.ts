...
function fromSeedPacket(p: SearchSeedPacket): SearchSeed | null {
  ...
  const parseKeywordHashes = (raw: unknown[]): SearchSeedKeywordHash[] | null => {
    ...
    for (let i = 0; i < raw.length; i++) {
      ...
      if (typeof r.count !== "number" || !Number.isFinite(r.count) || r.count <= 0) return null;
      out.push({ hash: r.hash >>> 0, count: r.count });
    }
    return out;
  };
...
function normalizeKeywordHashesFromBody(raw: unknown): SearchSeedKeywordHash[] | { error: string } {
  if (raw === undefined || raw === null) return [];
  if (!Array.isArray(raw)) return { error: "keywordHashes must be array if specified" };
  const counts = new Map<number, number>();
  for (let i = 0; i < raw.length; i++) {
    const v0 = raw[i] as unknown;
    if (typeof v0 === "number") {
      if (!Number.isFinite(v0)) return { error: `invalid keywordHashes[${i}]` };
      const h = (v0 >>> 0) as number;
      counts.set(h, (counts.get(h) ?? 0) + 1);
      continue;
    }
    if (!v0 || typeof v0 !== "object") return { error: `invalid keywordHashes[${i}]` };
    const r = v0 as Record<string, unknown>;
    const hash = r.hash;
    const count = r.count;
    if (typeof hash !== "number" || !Number.isFinite(hash))
      return { error: `invalid keywordHashes[${i}].hash` };
    if (typeof count !== "number" || !Number.isFinite(count) || count <= 0)
      return { error: `invalid keywordHashes[${i}].count` };
    const h = (hash >>> 0) as number;
    counts.set(h, (counts.get(h) ?? 0) + count);
  }
  return Array.from(counts.entries()).map(([hash, count]) => ({ hash, count }));
}

function keywordHashesFromSummary(summary: AiPostSummary): SearchSeedKeywordHash[] {
  const raw = Array.isArray(summary.keywordHashes) ? summary.keywordHashes : [];
  const counts = new Map<number, number>();
  for (const x of raw) {
    if (typeof x !== "number" || !Number.isFinite(x)) continue;
    const h = (x >>> 0) as number;
    counts.set(h, (counts.get(h) ?? 0) + 1);
  }
  return Array.from(counts.entries()).map(([hash, count]) => ({ hash, count }));
}
...
export default function createAiPostsRouter(
  ...
  router.post("/recommendations", async (req, res) => {
    ...
    const keywordHashesParsed = normalizeKeywordHashesFromBody(
      (b as Record<string, unknown>).keywordHashes,
    );
    if (!Array.isArray(keywordHashesParsed)) {
      return res.status(400).json({ error: keywordHashesParsed.error });
    }
    const keywordHashes = keywordHashesParsed;
  ...
  router.get("/recommendations/posts/for-post/:postId", async (req, res) => {
    ...
    try {
      ...
      if (!ids) {
        ...
        const keywordHashes = keywordHashesFromSummary(summary);
        if (targetWeights.size === 0 && keywordHashes.length === 0) {
          await redis.set(recKey, JSON.stringify([]), "EX", 180);
          ids = [];
        } else {
