...
import { LinkSnippetInputError, LinkSnippetRateLimitError, LinkSnippetsService } from "../services/linkSnippets";
...
export default function createLinkSnippetsRouter(pgPool: Pool, redis: Redis) {
  ...
  const authHelpers = new AuthHelpers(new AuthService(pgPool, redis), new UsersService(pgPool, redis));
