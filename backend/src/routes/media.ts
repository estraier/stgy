...
function publicize(meta: StorageObjectMetadata) {
  return {
    ...meta,
    publicUrl: `${Config.STORAGE_PUBLIC_BASE_URL}/${encodeURIComponent(meta.bucket)}/${meta.key
      .split("/")
...
export default function createMediaRouter(pgClient: Client, redis: Redis) {
  ...
  const bucket = Config.MEDIA_IMAGE_BUCKET;
  ...
  router.get(/^\/([^/]+)\/images\/(.*)$/, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const msg = (e as Error).message || "not found";
