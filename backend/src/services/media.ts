...
export class MediaService {
  ...
  async getImageBytes(
    ...
    if (isKeyUnder(`staging/${pathUserId}`, cleaned)) throw new Error("cannot read staging via this endpoint");
  ...
  async deleteImage(
    ...
    const cleaned = decodeURIComponent(keyWithoutUserPrefix).replace(/^\/+/, "");
    if (!isKeyUnder(pathUserId, `${pathUserId}/${cleaned}`)) throw new Error("invalid key");
    if (isKeyUnder(`staging/${pathUserId}`, cleaned)) throw new Error("cannot delete staging via this endpoint");
    const key = `${pathUserId}/${cleaned}`;
