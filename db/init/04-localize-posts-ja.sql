...
投稿一覧の上端には、新規投稿フォームがあります。フォームに本文を書き込んで「Post」ボタンを押すと、記事が投稿されます。「Preview」ボタンを押すと、執筆中の返信のプレビューが表示されます。記事の本文はMarkdown形式です。また、記事の最後の行に「#abc, #def」などと書いてタグを指定できます。詳細については[投稿の書式](/posts/00000000-0000-0000-0002-000000000003)の記事を御覧ください。
