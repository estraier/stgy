...
スニペットをDBに保存するということは、「どう表示するか」というフロントエンド側のプレゼンテーションの知識をバックエンド側で持つことを意味するため、責任分割の観点では気持ち悪い。しかし、その気持ち悪さを飲んでもなおやる価値があるくらい、スニペットを保存して本文を分離することの効果は大きい。
...
なお、PostgreSQLでは、TOASTという機能があり、大きい（2KB以上）列データを暗黙的に圧縮したり別テーブルに移動したりして、個々のレコードが単一ページに収まるように努力してくれる。よって、明示的に分割しなくても、ある程度の最適化は勝手になされる。しかし、それでもなお、明示的に分割した方が良い。スニペットさえあればリスト取得時には本文は一切参照する必要がないので、短くても長くても別テーブルにした方が、主テーブルのページ読み出し量が少なくて済むからだ。
