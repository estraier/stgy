...
なお、PostgreSQLでは、TOASTという機能があり、2KB以上の大きい列データを暗黙的に圧縮したり別テーブルに移動したりして、個々のレコードが単一ページに収まるように努力してくれる。よって、明示的に分割しなくても、ある程度の最適化は勝手になされる。しかし、それでもなお、明示的に分割した方が良い。スニペットさえあればリスト取得時には本文は一切参照する必要がないので、短くても長くても別テーブルにした方が、主テーブルのページ読み出し量が少なくて済むからだ。
