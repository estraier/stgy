...
function processMertensMessage(message) {
  const width = Number(message.width);
  const height = Number(message.height);
  const imageBuffers = Array.isArray(message.imageBuffers) ? message.imageBuffers : [];
  ...
  const images = imageBuffers.map((buffer) => new Float32Array(buffer));
  const contrastWeight = Number.isFinite(message.contrastWeight) ? Number(message.contrastWeight) : 0;
...
function mergeMertensExposureFusion(images, width, height, contrastWeight, saturationWeight, exposureWeight) {
  const pixelCount = width * height;
  const weightSums = new Float32Array(pixelCount);

  // First pass: compute the full-resolution denominator for normalized Mertens
  // weights.  The configured HDR2 contrast weight is zero, but the helper also
  // supports a non-zero value so the implementation remains a complete Mertens
  // weight calculation rather than a special-case pixel blend.
  for (let imageIndex = 0; imageIndex < images.length; imageIndex += 1) {
...
function mertensPixelWeight(image, width, height, x, y, contrastWeight, saturationWeight, exposureWeight) {
  ...
  if (contrastWeight !== 0) {
    const contrast = Math.max(mertensContrast(image, width, height, x, y), 1e-12);
...
function mertensContrast(image, width, height, x, y) {
  const center = grayAt(image, width, height, x, y);
  const left = grayAt(image, width, height, Math.max(0, x - 1), y);
  const right = grayAt(image, width, height, Math.min(width - 1, x + 1), y);
  const up = grayAt(image, width, height, x, Math.max(0, y - 1));
  const down = grayAt(image, width, height, x, Math.min(height - 1, y + 1));
  return Math.abs(4 * center - left - right - up - down);
}

function grayAt(image, width, height, x, y) {
  const offset = (y * width + x) * 3;
  return 0.299 * clamp01(image[offset]) + 0.587 * clamp01(image[offset + 1]) + 0.114 * clamp01(image[offset + 2]);
}
